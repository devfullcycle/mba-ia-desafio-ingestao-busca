...
**Dispatch Context7 lookups in parallel.** At the start of plan drafting, issue one tool call per library mentioned in the decisions document, all in a single message. For example, for an auth phase that chose argon2, PyJWT, Flask-Mail, Pydantic, and SQLAlchemy, send five `mcp__context7__query-docs` calls in parallel. Do NOT look up libraries one-by-one as you write each SI — batch them upfront and refer back to the results.
...
| File | Layer | Verifies |
|------|-------|----------|
| test_file.py | Unit/Integration/E2E | what this file tests |
...
- [ ] Linting and type-checks pass (`<linting/mypy command for subproject>`)
- [ ] Import checks pass (`<import check command for subproject>`)
```
...
- When introducing new dependencies, list the package name with a version range compatible with the project's existing Python version. Example: `Install PyJWT>=2.8.0, SQLAlchemy>=2.0.0 (Python 3.9+ compatible)`. This prevents dependency conflicts during implementation.
...
1. **List the artifacts** created in the SI (database models, services, utilities, API endpoints/routes, middleware, configuration, and any other artifact types defined by the subproject's testing guide).
...
E2E tests are listed per endpoint group (one row per e2e test file — typically the set of endpoints sharing a feature/module), not per individual endpoint. If the SI creates no testable artifacts, the Tests section may be omitted.
...
| How to build it internally | **Technical actions** | `Hash password using bcrypt.hashpw(); verify with bcrypt.checkpw()` |
| Which test files verify it | **Tests section** | `test_auth_e2e.py \| E2E \| login 200, 401, 403` |
...
- When a phase spans multiple subprojects/modules, repeat the test/linting/import-check deliverables per subproject, each with its own command (e.g., `All SI tests pass in src/core (<command>)` and `All SI tests pass in src/api (<command>)`).
