...
2. **Testing guide skill** — for each target Python subproject/module, use the Skill tool to load `testing-guide-{subproject}` if present. It defines how to write and run tests (Unit, Integration, E2E) using pytest or other frameworks. The Tests section of each SI tells you **which** files to create; the testing guide tells you **how** to write them.
...
- **Branch check**: `git status` and `git branch --show-current`. If the current branch is `main` or `dev`, or if there are uncommitted changes touching files outside the target subproject's directory (e.g., outside `src/` when that is the target), stop and ask the user to set up the right branch first.
- **Subproject readiness**: the target subproject/module exists and its dependencies are installed. For Python projects: check if a virtual environment exists (`.venv/bin/python` or similar), or verify that dependencies can be installed (requirements.txt, pyproject.toml, or Poetry.lock). If not, ask the user to set it up first (e.g., `python -m venv .venv` or `poetry install`).
...
Re-read the SI section in full (Description, Technical actions, Tests, Dependencies, Acceptance criteria). Load relevant best-practices skills matching the artifacts this SI builds (e.g., skills for modules, API design, database operations, async programming) — load only what this SI needs, not all available skills. Keep a short internal checklist for this SI: one item per technical action; additionally, when the SI has a Tests section, one item per test file plus a "run tests" item. This is working memory to keep the SI on track — not a formal deliverable.
...
- **Fix the root cause.** Do not weaken tests to make them pass. Do not add skips (`@pytest.mark.skip`), conditionals, or exception swallowing. Do not catch and swallow errors just to hide them.
...
- **No shortcuts.** Never disable assertion checks or bypass safety mechanisms.
...
1. **Full test suite**: Run every test command listed in the plan's Deliverables (typically `pytest` or equivalent). The goal is to exercise every test in the phase together, not just the tests of a single SI.
2. **Linting and type-checking**: Run the linting/type-check commands defined in the plan's Deliverables (e.g., `mypy`, `pylint`, `flake8`, or similar) to verify code quality and type safety.
3. **Import checks**: Verify that all modules import correctly and there are no circular dependencies (e.g., run a quick import check or `python -m py_compile`).

Verify any additional deliverables listed in the plan (database migrations, documentation updates, configuration files, seed data). These results are presented in the **Completion report** (see section below).

If the phase spans multiple subprojects/modules, the plan's Deliverables checklist will list these checks per subproject (e.g., `All SI tests pass in src/core` and `All SI tests pass in src/api`) — run each subproject's commands independently, in the order implied by the checklist.
